...
{(","+newline).join(select_exprs)}
    ...
    def _gen_sort(self, snap: Dict) -> str:
        ...
        return f'''{var} = df.orderBy(F.col("column_name").asc())'''
    
    def _gen_aggregate(self, snap: Dict) -> str:
        var = self._get_var_name("df_agg")
        
        return f'''{var} = df.groupBy("group_column").agg(
...
df_route_2 = df.filter(F.expr("condition_2"))'''
    ...
    def _gen_script(self, snap: Dict) -> str:
        ...
        return f'''# Script Snap - Original script:
\"\"\"
{script}
\"\"\"
# TODO: Convert to PySpark/Python'''
    ...
    def _gen_expression(self, snap: Dict) -> str:
        ...
        return f'''{var} = df.withColumn("result", F.expr("{expr}"))'''
    ...
    def _gen_pipeline_execute(self, snap: Dict) -> str:
        ...
        return f'''# Pipeline Execute - Calls: {pipeline_path}
result = dbutils.notebook.run(
    ...
    def _gen_gate(self, snap: Dict) -> str:
        ...
        return f'''# Gate Snap - Conditional flow
if {condition}:
    pass
else:
...
# Snap Name: {snap_name}
# This snap type requires manual conversion.
# Original properties: {snap.get("properties", {})}'''
